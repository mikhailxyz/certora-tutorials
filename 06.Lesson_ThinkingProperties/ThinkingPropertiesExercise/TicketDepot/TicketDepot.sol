...
contract TicketDepot {
    ...
    function buyNewTicket(
        uint16 _eventID,
        address _attendee
    ) public payable ticketsAvailable(_eventID) returns (uint16 ticketID) {
        if (
            msg.sender == eventsMap[_eventID].owner ||
        ...
        payable(eventsMap[_eventID].owner).transfer(
            eventsMap[_eventID].ticketPrice
        );
    ...
    function offerTicket(
        ...
        if (
            msg.sender != eventsMap[_eventID].attendees[_ticketID] ||
    ...
    function buyOfferedTicket(
        ...
        if (
            ...
            (offerings[offerID].buyer != address(0) &&
                offerings[offerID].buyer != msg.sender)
        ) revert();

        // paying the current ticket holder (suggester)
        payable(eventsMap[_eventID].attendees[_ticketID]).transfer(
